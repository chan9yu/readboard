...
**MVP 상태: 전체 Phase 완료 -- 모든 기능(F001~F004) 구현 및 검증 완료**

## 페이즈 요약

| Phase   | 목표                     | 주요 기능                                     | 복잡도 | 선행 조건                         | 상태 |
| ------- | ------------------------ | --------------------------------------------- | ------ | --------------------------------- | ---- |
| Phase 1 | 기반 구조 및 타입 시스템 | 프로젝트 스캐폴딩, 타입 정의                  | Low    | 없음                              | 완료 |
| Phase 2 | Notion API 연동          | SDK 설치, API 클라이언트, 데이터 매퍼         | Medium | Phase 1                           | 완료 |
| Phase 3 | 카드 UI 컴포넌트         | ReadingCard, ReadingCardSkeleton, EmptyState  | Medium | Phase 1 (**Phase 2와 병렬 가능**) | 완료 |
| Phase 4 | 필터링 및 보드 컴포넌트  | StatusTabs, ReadingBoard, URL 쿼리 연동       | High   | Phase 3                           | 완료 |
| Phase 5 | 페이지 통합 및 완성      | page.tsx 통합, loading.tsx 연결, 통계 요약    | Medium | Phase 2 + Phase 4                 | 완료 |
| Phase 6 | 검증 및 품질 보증        | Playwright 테스트, 반응형 검증, 에러 시나리오 | Medium | Phase 5                           | 완료 |
...
- [x] Task 006: `@notionhq/client` 패키지 설치 (`pnpm add @notionhq/client`)
- [x] Task 007: `.env.local`에 `NOTION_API_KEY`, `NOTION_DATABASE_ID` 환경 변수 설정 및 `.env.example` 파일 생성
- [x] Task 008: `notionMapper.ts`에 `mapPageToReadingItem()` 함수 구현 (`src/features/reading/services/notionMapper.ts`)
  - Notion `PageObjectResponse`를 `ReadingItem`으로 변환
  - 한글 속성명(`제목`, `저자`, `상태`, `평점`, `표지 이미지`, `카테고리`, `등록일`) 매핑
  - 누락 필드에 대한 null 폴백 처리
- [x] Task 009: `notionClient.ts`에 Notion SDK 초기화 및 `fetchReadingList()` 함수 구현 (`src/features/reading/services/notionClient.ts`)
  - `use cache` 디렉티브 적용
  - `cacheLife("hours")` 설정 (1시간 재검증)
  - `등록일` 기준 내림차순 정렬, `page_size: 100`
  - `mapPageToReadingItem` 호출하여 결과 변환
- [x] Task 010: 환경 변수 미설정 시 빌드 타임 오류를 발생시키는 검증 로직 추가 (`src/features/reading/services/notionClient.ts`)
...
- Rate Limiting: 슬라이딩 윈도우 방식으로 분당 30회 제한 (`src/shared/utils/rateLimiter.ts`)
- 구조화된 로깅: JSON 포맷의 `logger` 유틸로 API 호출 성공/실패/제한 초과를 추적한다 (`src/shared/utils/logger.ts`)

### Playwright 테스트 (API 연동 Task 체크리스트)

- [x] Task 011: Notion API 연동 후 페이지 접속 시 데이터가 렌더링되는지 기본 검증 (Phase 6에서 실행 완료)
  - [x] Happy Path: 정상 API 응답 시 카드 목록이 올바르게 렌더링되는지 확인
  - [x] 에러 시나리오: API 키 누락/잘못된 DB ID 시 에러 UI 표시 확인
  - [x] 엣지 케이스: 빈 DB(항목 0개) 시 EmptyState 표시 확인
  - [x] 로딩 상태: 데이터 fetching 중 스켈레톤 UI 표시 확인

---

## Phase 3: 카드 UI 컴포넌트 (완료)
...
- [x] Task 012: `ReadingCard` 컴포넌트 구현 (`src/features/reading/components/ReadingCard.tsx`)
...
- [x] Task 013: `ReadingCardSkeleton` 컴포넌트 구현 (`src/features/reading/components/ReadingCardSkeleton.tsx`)
  - `Skeleton` 컴포넌트(`src/shared/ui/Skeleton.tsx`) 재사용
  - `ReadingCard`와 동일한 높이/레이아웃 구조 유지
  - 이미지 영역, 제목, 저자, Badge 위치에 스켈레톤 배치
- [x] Task 014: `EmptyState` 컴포넌트 구현 보강 (`src/features/reading/components/EmptyState.tsx`)
...
- `StarRating`은 `ReadingCard`에서 분리된 별점 표시 컴포넌트이다
...
- [x] Task 015: `StatusTabs` 컴포넌트 구현 (`src/features/reading/components/StatusTabs.tsx`)
...
- [x] Task 016: `ReadingBoard` 컴포넌트 구현 (`src/features/reading/components/ReadingBoard.tsx`)
...
- [x] Task 017: `ReadingBoard`를 `Suspense` 경계로 감싸는 구조 확인 (`src/app/page.tsx`)
  - `useSearchParams()` 사용 컴포넌트는 `<Suspense>`로 감싸야 정적 생성이 보장됨 (PRD F002 비즈니스 규칙)

### 기술 노트

- 필터링은 서버 재호출 없이 클라이언트에서 수행한다 (전체 데이터를 props로 받아 필터링)
- 필터링 유틸리티 함수(`isValidStatusFilter`, `computeStatusCounts`, `filterItems`)는 `utils/statusFilter.ts`로 분리되어 있다
- `StatusTabItem`은 `StatusTabs`에서 분리된 개별 탭 버튼 컴포넌트이다
...
- [x] Task 018: 탭 클릭 시 URL 쿼리 파라미터 변경 및 카드 필터링 검증
  - [x] 입력 검증: 4개 탭(전체/읽는중/완독/읽을예정) 각각 클릭 시 올바른 필터링
  - [x] 상태 전이: 탭 전환 시 URL `?status=` 파라미터 정확히 변경되는지 확인
  - [x] 데이터 변환: 필터링된 카드 수와 탭의 카운트 숫자 일치 확인
- [x] Task 019: 유효하지 않은 `?status=invalid` 접속 시 "전체" 탭 폴백 검증
  - [x] 엣지 케이스: 빈 문자열, 숫자, 특수문자 등 다양한 무효 값 테스트
- [x] Task 020: 특정 `?status=done` URL 직접 접속 시 해당 필터 유지 검증
  - [x] Happy Path: URL 공유 링크로 직접 접속 시 필터 상태 유지
  - [x] 새로고침 시 필터 상태 유지 확인

---

## Phase 5: 페이지 통합 및 완성 (완료)
...
- [x] Task 021: `page.tsx` 통합 완성 (`src/app/page.tsx`)
  - `fetchReadingList()` 호출 후 `ReadingBoard`에 데이터 전달 (완료)
  - 통계 요약 렌더링: "N권 완독 / N권 읽는중" (서버에서 items 기반 계산) (완료)
  - `Suspense` fallback을 `loading.tsx`의 스켈레톤 패턴과 일치시키기 (완료)
- [x] Task 022: `loading.tsx`에서 `ReadingCardSkeleton` 12개 그리드 렌더링 확인 및 실제 카드 레이아웃과 정합성 검증 (`src/app/loading.tsx`)
- [x] Task 023: `error.tsx` 에러 UI에서 "다시 시도" 버튼 동작 검증 (`src/app/error.tsx`)
- [x] Task 024: Header 컴포넌트에 서비스 타이틀 및 다크모드 토글 반영 확인 (`src/shared/layouts/Header.tsx`)
- [x] Task 025: Footer 컴포넌트에 저작권 표시 반영 확인 (`src/shared/layouts/Footer.tsx`)

### 기술 노트

- `page.tsx`는 Server Component로 유지하며 데이터 패칭을 직접 수행한다
- 통계 요약은 `StatsSummary` 컴포넌트로 분리하여 `page.tsx`에서 사용한다
- `Suspense` fallback과 `loading.tsx`의 역할이 중복될 수 있으므로 정리가 필요하다

---

## Phase 6: 검증 및 품질 보증 (완료)
...
- [x] Task 026: `pnpm build` 프로덕션 빌드 성공 검증
- [x] Task 027: `pnpm type:check` TypeScript 타입 검사 통과 검증
- [x] Task 028: `pnpm lint` ESLint 검사 통과 검증

### Playwright 테스트 (E2E 사용자 플로우 + 에러/엣지 케이스)

- [x] Task 029: F001 검증 -- 페이지 접속 시 카드 렌더링 및 등록일 내림차순 정렬 확인
  - [x] Happy Path: Notion DB 데이터가 카드 그리드로 정상 표시
  - [x] 데이터 정합성: 제목, 저자, 상태, 평점, 카테고리가 Notion 원본과 일치
- [x] Task 030: F002 검증 -- 4개 탭 각각 클릭 시 필터링 동작 확인
  - [x] 각 탭 클릭 -> 해당 상태 카드만 표시 -> 카운트 일치
- [x] Task 031: F003 검증 -- 초기 로드 시 스켈레톤 UI 표시 후 실제 카드로 전환 확인
- [x] Task 032: F004 검증 -- 뷰포트 크기별 그리드 열 수 확인 (모바일 1열, 태블릿 2열, 데스크톱 3열, 와이드 4열)
  - [x] 각 뷰포트에서 browser_resize -> snapshot -> 열 수 검증
- [x] Task 033: 빈 DB 시나리오 -- EmptyState 메시지 표시 확인
  - [x] 전체 탭: "아직 등록된 항목이 없습니다" 메시지
  - [x] 특정 상태 탭: "이 상태의 항목이 없습니다" 메시지
- [x] Task 034: 다크모드 전환 시 모든 컴포넌트의 시각적 정합성 확인
- [x] Task 035: Static Analysis 최종 검증 -- `pnpm type:check` + `pnpm lint` 통과 (테스트 트로피 기반층)
...
| 리스크                      | 영향도               | 완화 방안                                                                               |
| --------------------------- | -------------------- | --------------------------------------------------------------------------------------- |
| Notion API 속도 저하        | 초기 로드 지연       | `use cache` + `cacheLife("hours")` 캐싱 + Rate Limiting(분당 30회)으로 반복 호출 최소화 |
